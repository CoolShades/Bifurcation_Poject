...
foreach(i=1:5, .packages = c("foreach", "tidyverse", "gtsummary")) %dopar% {
...
match.it <- matchit(DCB_DES ~ Age+ Sex+ Presentation+ BIF_Medina_Classification+ True_Bif+ Frailty_Sc+ Duration_Procedure+ PMHx_Hyperchol+ PMHx_HTN+ PMHx_PVD+ PMHx_CVE+ PMHx_MI+ PMHx_CABG+ PMHx_HF+ PMHx_Angina+ FHx_CAD+ PMHx_Asthma+ PMHx_COPD+ PMHx_DM+ Smok_Stat+ Creatinine+ Vessel_Tx+ Heavy_Calc+ Tortuosity
                    , data = data_ps_bif, method="optimal", ratio=1) #Full matching technique
a <- summary(match.it)
...
step_reg <- future_map(1:5, ~stats::step(res.cox[[.x]], direction='both', scope=list(lower = Surv(time = !!sym(Followups[.x]), event = !!sym(Outcomes[.x])) ~ DCB_DES, upper = stats::formula(res.cox[[.x]])), trace=0))

MV_T <- map(1:5, ~gtsummary::tbl_regression(step_reg[[.x]], 
               exponentiate = T, 
...
map(1:5, ~gt::gtsave(MV_T[[.x]], paste0("MV_Tables/" , Outcomes[.x], "_MV.html")))
